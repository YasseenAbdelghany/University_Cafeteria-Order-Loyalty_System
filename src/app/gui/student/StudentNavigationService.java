...
/**
 * Navigation service for managing scene transitions in the Student Portal application.
 * This class handles loading FXML files from the student directory, initializing controllers,
 * and switching between scenes with caching support.
 */
public class StudentNavigationService {
    private static final Logger logger = Logger.getLogger(StudentNavigationService.class.getName());
    ...
    private StudentNavigationService() {}
    ...
    public static void initialize(Stage primaryStage, ServiceContainer services) {
        ...
        logger.info("StudentNavigationService initialized.");
    }
    ...
    public static void navigateToWithData(String sceneName, Object data) {
        try {
            ...
            // Refresh controller if it has a refresh method (e.g., for notification badge)
            if (controller != null) {
                refreshController(controller, sceneName);
            }
    ...
    private static Scene loadScene(String sceneName) throws IOException {
        ...
        // Load from FXML file in student directory
        String fxmlPath = "/app/resources/fxml/student/" + sceneName + ".fxml";
        logger.info("Loading FXML from: " + fxmlPath);
        
        FXMLLoader loader = new FXMLLoader(StudentNavigationService.class.getResource(fxmlPath));
    ...
    /**
     * Refresh a controller when navigating back to it.
     * This is used to update dynamic content like notification badges.
     * 
     * @param controller The controller instance
     * @param sceneName The name of the scene being navigated to
     */
    private static void refreshController(Object controller, String sceneName) {
        try {
            // Special handling for menu-dashboard to refresh notification badge
            if ("menu-dashboard".equals(sceneName)) {
                controller.getClass().getMethod("refreshNotificationBadge").invoke(controller);
                logger.info("Refreshed notification badge for menu-dashboard");
            }
        } catch (NoSuchMethodException e) {
            // Method doesn't exist, which is fine for controllers that don't need refresh
        } catch (Exception e) {
            logger.warning("Could not refresh controller: " + e.getMessage());
        }
    }
    ...
    /**
     * Get a cached controller by scene name
     *
